...
\begin{figure}[h]
\centering
\includegraphics[width=0.18\textwidth]{pictures/elte.jpg}
\end{figure}
...
Ez a téma az 1950-es közepétõl vált kutatott területté \cite{fiedler1973algebraic,donath-hoffman73}.
...
A rangsort az oldalak közt levõ hiperlinkek alapján állítja fel.
...
Ha $h^{(1)}$-t úgy választjuk, hogy merõleges legyen $U$ elsõ oszlopára, akkor a numerikus hibáktól eltekintve az iterációk során végig a merõleges altérben maradunk, azaz elimináljuk az elsõ szinguláris vektor hatását. Ilyenkor a második lesz a hangsúlyos szinguláris vektor, és a megfelelõ normalizációs konstans $\sigma_2$ lesz.

A fenti gondolatmenet alapján a HITS-algoritmus alkalmas a szinguláris felbontás meghatározására.
Ha már ismert az elsõ $k$ szinguláris vektor, akkor választhatunk rájuk merõleges kiinduló vektort. Ekkor a HITS normalizálás után a $(k+1)$-edik szinguláris vektorpárhoz fog konvergálni.
...
%\end{array} \right.\]
%\end{defin}
A dolgozat hátralevõ részében olyan egyszerû súlyozott gráfokkal foglalkozunk, melyekhez adva van egy $w:V\times V \rightarrow \mathbb{R}$ súlyfüggvény, amelyre igazak az alábbiak:
\[w(u,v)=w(v,u)\]
\[w(u,v) \geq 0\]
\[uv \textrm{ nem él} \Leftrightarrow w(u,v)=0\]
A $w(u,v)$ értékeket mátrixba rendezve kapjuk a $W$ mátrixot.
Vegyük észre, hogy a $W$ önmagában meghatározza a gráfot, így a súlyozott gráfokat a $V$ csúcshalmazzal és a $W\geq 0$ súlymátrixszal fogjuk megadni.

A súlyozott gráfok esetében definiálhatunk egy általánosabb fokszámfogalmat.
\begin{defin}[Általánosított fokszám]
$G=(V,W)$ súlyozott gráf egy $v$ csúcsának általánosított fokszáma
\[d_v=\sum_{w(u,v) > 0} w(u,v).\]
\end{defin}
Most már definiálhatjuk egy súlyozott gráf Laplace-mátrixát.
\begin{defin}[Laplace-mátrix]
Egy $G=(V,W)$ gráf Laplace-mátrixa
\[L(u,v)=\left\{
\begin{array}{cc}
...
A következõkben megmutatjuk a Laplace mátrix legfontosabb tulajdonságait. Legelõször bebiyonyítjuk, hogy  $\mathcal{L}$ szimmetrikus pozitív-szemidefinit, azaz az összes sajátértéke valós nemnegatív.  A szimmetria abból következik, hogy a gráf irányítatlan.

A pozitív szemidefinitség bizonyításához vegyünk egy tetszõleges $g:V\rightarrow\mathbb{R}$ függvényt és tekintsük, mint oszlopvektort.  Definiáljuk az ún. \textit{Rayleigh-hányadost}:
\begin{eqnarray}
...
$\mathbb{R}^n$-beli skalárszorzatot.

A \eqref{Rayleigh} egyenlõségbõl következik, hogy az összes sajátérték nemnegatív és a $0$ is sajátérték. A $0$-hoz tartozó sajátvektor a
...
$X\in \mathbb{R}^{n\times k}$ adott szubortogonális mátrix. $X$ oszlopait az $x_1,\dots,x_k$ vektorokkal, sorait pedig az $r^T_1,\dots,r^T_n$ vektorokkal jelöljük.
\begin{eqnarray}
\nonumber
...
Nem teljes gráfra azonban már az is igaz, hogy $\lambda_1 \leq 1$, lásd \cite{chung1997spectral}.

\subsection{Néhány speciális gráf spektruma}
$\mathbf{K_n}.$
Az $n$ csúcsú teljes gráf spektruma 
\[\lambda_0 = 0, \lambda_1 = \dots = \lambda_{n-1} = \frac{n}{n-1},\] 
mivel a gráf teljesen szimmetrikus és mert $tr \mathcal{L} = n$ \cite{bolla}.
Ebbõl látható, hogy a \eqref{eq:lambdaAtlag} becslés éles.

$\mathbf{K_{m,n}}.$
Az $m$ és $n$ elemû osztályokból álló teljes páros gráf spektruma \cite{bolla} szerint
...
$\mathbf{P_n}.$
Az $n$ hosszú út spektruma \cite{cvetkovic1980spectra} szerint
\[\lambda_k= 1-\cos\frac{\pi k}{n-1} \quad k=0,\dots,n-1.\]

$\mathbf{C_n}.$
Az $n$ hosszú kör spektruma \cite{chung1997spectral} szerint:
\[\lambda_k=1-\cos\frac{2\pi k}{n} \quad k=0,\dots,n-1.\] 

$\mathbf{Q_n}.$
A $2^n$ csúcsú $n$ dimenziós hiperkocka sajátértékei \cite{lovasz2007combinatorial} szerint
...
Elõkerülhetnek rejtett kapcsolatok, ami a gráf struktúrájának jobb megértéséhez vezet. Newman \cite{newman04detecting} szerint a hálózatok kutatásának kiemelten fontos témája a közösségek, azaz sûrû részgráfok vizsgálata. 
...
Partícionálás során minden csúcsot be kell sorolnunk egy klaszterbe.
Tehát a klaszterezés egy $k$-részre vágás, vagyis a $G=(V,W)$ gráf csúcsainak egy olyan $P_k=(V_1,\dots,V_k)$ felosztása, ahol $V_i$-k diszjunktak és 
\[\bigcup_{i=1}^k V_i =V.\] 
$k=2$ esetén bipartícionálásról beszélünk. 

A megfelelõ partícionálás matematikailag nehezen megfogható fogalom. Az általunk használt mértékekhez sok esetben könnyen találunk olyan példát, amikor az optimális partícionálás messze van az elvárttól. Egy jó partícionálástól elvárjuk, hogy az egy csoportban levõ elemek hasonlítsanak egymásra, míg a különbözõ csoportokban levõk különbözzenek. Valamint elvárt az is, hogy a klaszterek hasonló méretûek legyenek.
...
\begin{defin}[Vágás értéke] 
A $G=(V,W)$ élsúlyozott gráfhoz tartozó $S,T \in V$ nem üres csúcshalmazok által meghatározott vágás súlya \cite{ford1962flows} alapján
\[w(S,T)=\sum_{s\in S}\sum_{t\in T}w_{st}.\]
\end{defin}
A fenti definíciót kiterjeszthetjük tetszõleges számú halmazra.
...
Legyen $U^*$ egy minimum vágás. Az $U$ segítségével definiáljuk az alábbi egydimenziós reprezentánsokat.
...
Fiedler \cite{fiedler1973algebraic} egy egyszerû gráfra a minimum vágás értékét élösszefüggõségnek hívta, mivel ennyi élt kell eltávolítani ahhoz, hogy a gráf ne legyen összefüggõ. Az élösszefüggõségre az $e(G)$ jelölést vezette be. Mivel a $\lambda_1$ sajátérték és az élösszefüggés között szoros kétoldali kapcsolat van, a $\lambda_1$ sajátértéket elnevezte algebrai összefüggõségnek. Megmutatta, hogy nem teljes gráfra 
\[\lambda_1\leq e(G).\]
...
Amint azonban az \ref{fig:mincut}.~ábrán is látható, könnyen elõfordulhat, hogy egy minimális vágás csak egy csúcsot vág le a gráfról, és valójában nem ez a ,,legjobb'' vágás.
...
A \ref{fig:diameter}.~ábra jól demonstrálja azt az esetet, amikor az átmérõvel nem a  kívánt partíciókat kapjuk.

\begin{figure}[h!]
\centering
\includegraphics[scale=0.1]{pictures/diameter-fail.jpg}
\caption{Példa arra, amikor a maximális átmérõ nem ad optimális vágást \cite{kannan2004clusterings}.}
\label{fig:diameter}
\end{figure}
...
Elõször azzal a kérdéssel foglalkozunk, amikor bipartícionálni szeretnénk $G$-t.
...
Az 1. feladat megoldása egyenértékû a Cheeger-konstans meghatározásával, mivel \cite{chung1997spectral} szerint
...
\end{equation}
%\[\sum_v g(v)^2 d_v=1.\]
Feltehetõ, hogy a csúcsok $f$ szerint csökkenõ sorrendbe vannak rendezve vagyis $f(v_i) \geq f(v_j)\textrm{, ha } i<j$. Legyen $S_i=\{v_1,\dots, v_i\}$, valamint
\[\alpha_G=\min_i h_{S_i}.\]
...
\end{eqnarray*}
A Cauchy-Schwarz-Bunyakovszkij egyenlõtlenség miatt tovább
\begin{eqnarray*}
\lambda_1 &\geq& \frac{\Big(\sum_{u\sim v}\big(g_+(u)^2-g_+(v)^2\big)\Big)^2}{2\left (\sum_v g_+^2(v)d_v\right )^2}
\\
&=&\frac{\Big(\sum_i |g_+(v_i)^2-g_+(v_{i+1})^2|\,|\partial(S_i)|\Big)^2}{2\Big(\sum_v g_+(v)^2d_v\Big)^2}.
\end{eqnarray*}
Bevezetve a
\[\tilde{Vol}(S)=\min(Vol(S), Vol(G)-Vol(S))\] 
jelölést és használva az $\alpha_g$ definícióját, azt kapjuk, hogy
\begin{eqnarray*}
\lambda_1 &\geq& \frac{\Big(\sum_i |g_+(v_i)^2-g_+(v_{i+1})^2|\alpha_G|\tilde{Vol}(S_i)|\Big)^2}{\Big(\sum_v g_+(v)^2d_v\Big)^2}
\\
...
\end{biz}
A lemma bizonyításával beláttuk a Cheeger-egyenlõtlenség másik irányát is. 
\end{biz}
...
A következõkben áttérünk a multipartíciók vizsgálatára. Gráfok több részre való partícionálásának az egyik kedvelt heurisztikus megoldása a spektrál módszer \cite{ding01minmax,meila01random,zha01spectral}.
A másik kedvelt módszer az agglomeratív klaszterezés \cite{alpert-survey}, amelyrõl azonban ismert, hogy instabil megoldást ad \cite{hopcroft03communities}, ezért ezzel az iránnyal nem foglalkozunk. 
...
\begin{tetel}[Reprezentációs tétel súlyozott gráfokra]
...
.\]
Az optimum az $X=\left(u_1,\dots,u_k\right)$ szubortogonális mátrix, ahol $u_i$ az $\lambda_i$-hez tartozó egységhosszú sajátvektor \cite{bolla}. \end{allitas}
\end{biz}
...
A TKC jelenség elméleti alapját az adja, hogy a mátrix spektruma a \ref{sect:hits}.~fejezet végén látott út-leszámlálás mennyiséggel függ össze. A következõ állítások szerint a sûrû részgráfokban exponenciálisan gyorsan nõ az utak száma, így a sajátvektorokat ``magukhoz vonzzák''. 
...
Ebben a részben két heurisztikát szeretnénk ismertetni,
...
Csápoknak nevezzük egy elõre rögzített $d_t$ fokszámnál kisebb fokú csúcsokból álló részgráfot. Ezek eltávolítására a következõ eljárást alkalmazhatjuk.
\begin{enumerate}
...
Példánkban \cite{kurucz} cél a LiveJournal (www.livejournal.com) oldal felhasználóinak klaszterezése.
...
Az alábbi ábrán a gráf egy részlete látható, amely jól szemlélteti, hogy a gráf struktúrájában hogy helyezkednek el a csápok(zöld részek) és csomók(piros részek).

\begin{figure}[h]
\centering
\includegraphics[scale=0.5]{pictures/livejournal.jpg}
\caption{A livejournal.com barátsággráfjának egy részlete}
\end{figure}
...
A gráfból elõször eltávolították a csomópontokat, majd alkalmazták a csápok összehúzását. Innen kétféleképp haladtak tovább. Egyrészt Shi és Malik algoritmusát alkalmazták . Másrészt Lang \cite{lang2005fixing} algoritmusát is megvalósították, aki a \ref{thm:rep}.~Tételben ismertetett reprezentáció meghatározására egy szemidefinit relaxált programot old meg.
Végül a kivett csomókat viszarakva, és a csápokat visszanövesztve kapták a végsõ klasztereket. A spektrálklaszterezést lefuttatták elõfeldolgozás nélkül is, az összehasonlításhoz. 
\subsubsection{Eredmények}
Az eredményeket a \ref{fig:livejournal}--\ref{fig:semidef}.~ábrák mutatják be.
...
Látható, hogy  az elõfeldolgozás nélkül a két csoportot nem lehet megkülönböztetni. A heurisztikák alkalmazása után a klaszterek azonban eltávolodnak egymástól.

Az egyes klaszterek minõségét az entrópiával és tisztasággal mérték.
\begin{defin}[Entrópia]
Egy $X$ halmaz $X_1, \dots , X_j$ felbontásának entrópiája 
\[h(X) = -\sum_i p_i \log p_i,\]
ahol $p_i = |X_i|/|X|.$
\end{defin}

\begin{defin}[Tisztaság]
Egy $X$ halmaz $X_1, \dots , X_j$ felbontásának tisztasága 
\[t(X) = \max_i p_i, \]
ahol $p_i = \frac{|X_i|}{|X|}.$
\end{defin}
...
\hline 
 & Futási idõ(perc) & Entrópia & Tisztaság \\ 
\hline 
SVD elõfeldolgozás nélkül & 1980 & 0,105 & 0,812 \\ 
\hline 
SVD elõfeldolgozással & 150 & 0,073 & 0,853 \\ 
\hline 
SDP elõfeldolgzás nélkül & 1755 & 0,111 & 0,857 \\ 
\hline 
SDP elõfeldolgozással & 675 & 0,072 & 0,854 \\ 
\hline 
\end{tabular} 

\caption{A spektrál heurisztikák minõsége és futási ideje a LiveJournal orosz részgráfján.}
\label{tab:spectral}
\end{table}

Amint az \ref{tab:spectral}.~táblázatban látható, szemidefinit programmal összehasonlítható lett az eredmény, míg a spektrálklaszterezés sokkal gyorsabban futott.
...
A jelenleg ismertebb véletlen gráf modellek, mint a Barabási-Albert-modell, rosszul modellezik a valóságban létezõ nagy gráfokat a spektrálklaszterezés nehézségeinek tekintetében.
